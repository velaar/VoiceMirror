...
/**
 * @brief The VoicemeeterManager class encapsulates interactions with the
 * Voicemeeter API.
 *
 * This class handles initialization, shutdown, and provides utility functions
 * to interact with Voicemeeter. It ensures that the API is properly managed and
...
class VoicemeeterManager {
   public:
     ...
     *
     * @param voicemeeterType The type of Voicemeeter to initialize (1:
     * Voicemeeter, 2: Banana, 3: Potato).
     ...
     * specified duration before and after restarting the audio engine, and logs
     * the process.
     *
     ...
     * seconds).
     */
    void RestartAudioEngine(int beforeRestartDelay = 2,
                            int afterRestartDelay = 2);
     ...
     *
     * This function retrieves and logs all input strips and output buses,
   ...
   private:
    VoicemeeterAPI
        vmrAPI;              ///< Instance of VoicemeeterAPI to interact with Voicemeeter.
