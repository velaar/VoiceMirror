    ...
    : targetDeviceUUID(deviceUUID), toggleConfig(toggleConfig), voicemeeterManager(manager), debugMode(manager.GetDebugMode()), volumeMirror(mirror), refCount(1), isToggled(false)
{
    ...
    if (debugMode)
    { // debug
        Logger::Instance().Log(LogLevel::DEBUG, "DeviceMonitor initialized for UUID: " + targetDeviceUUID);
    }
}
...
DeviceMonitor::~DeviceMonitor()
{
    ...
    if (debugMode)
    {
        Logger::Instance().Log(LogLevel::DEBUG, "DeviceMonitor destroyed for UUID: " + targetDeviceUUID);
    }
}
...
STDMETHODIMP DeviceMonitor::OnDeviceStateChanged(LPCWSTR pwstrDeviceId, DWORD dwNewState)
{
    if (debugMode)
    {
        Logger::Instance().Log(LogLevel::DEBUG, "OnDeviceStateChanged called for Device ID: " + std::to_string(dwNewState));
    }
...
void DeviceMonitor::CheckDevice(LPCWSTR deviceId, bool isAdded)
{
    ...
    if (debugMode)
    {
        Logger::Instance().Log(LogLevel::DEBUG, "Checking device UUID: " + deviceUUID);
    }
...
void DeviceMonitor::HandleDevicePluggedIn()
{
    ...
    Logger::Instance().Log(LogLevel::INFO, "Monitored device has been plugged in.");
...
{
    std::lock_guard<std::mutex> lock(toggleMutex);
    Logger::Instance().Log(LogLevel::INFO, "Monitored device has been unplugged.");
...
void DeviceMonitor::ToggleMute(const std::string &type, int index1, int index2, bool isPluggedIn)
{
    ...
    if (isPluggedIn)
    {
        ...
        Logger::Instance().Log(LogLevel::INFO, "Device Plugged: Unmuted " + type + ":" + std::to_string(index1) + ", Muted " + type + ":" + std::to_string(index2));
        isToggled = true;
    }
    else
    {
        ...
        Logger::Instance().Log(LogLevel::INFO, "Device Unplugged: Muted " + type + ":" + std::to_string(index1) + ", Unmuted " + type + ":" + std::to_string(index2));
        isToggled = false;
    }
}