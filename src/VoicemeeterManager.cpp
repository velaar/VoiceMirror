...
bool VoicemeeterManager::Initialize(int voicemeeterType) {
    if (loggedIn) {
        Logger::Instance().Log(LogLevel::DEBUG,
                               "Voicemeeter is already logged in.");
        return true;
    }

    Logger::Instance().Log(LogLevel::DEBUG,
                           "Initializing Voicemeeter Manager...");

    if (!vmrAPI.Initialize()) {
        Logger::Instance().Log(LogLevel::ERR,
                               "Failed to initialize Voicemeeter API.");
        return false;
    }

    long loginResult = vmrAPI.Login();
    Logger::Instance().Log(LogLevel::DEBUG, "Voicemeeter login result: " +
    ...
    if (loggedIn) {
        ...
        Logger::Instance().Log(LogLevel::DEBUG,
                               "GetVoicemeeterType result: " + std::to_string(hr) +
                                   ", Type: " + std::to_string(vmType));
        if (hr != 0) {
            Logger::Instance().Log(
                LogLevel::WARNING,
                "Voicemeeter is not running. Attempting to start it.");
            loggedIn = false;
        }
    }
    ...
    if (!loggedIn) {
        Logger::Instance().Log(
            LogLevel::WARNING,
            "Voicemeeter login failed, attempting to run Voicemeeter Type:" +
                std::to_string(voicemeeterType));
        long runResult = vmrAPI.RunVoicemeeter(voicemeeterType);
        Logger::Instance().Log(LogLevel::DEBUG, "RunVoicemeeter result: " +
                                                    std::to_string(runResult));

        if (runResult != 0) {
            Logger::Instance().Log(LogLevel::ERR,
                                   "Failed to run Voicemeeter. Error code: " +
                                       std::to_string(runResult));
            return false;
        }
        ...
        Logger::Instance().Log(LogLevel::DEBUG,
                               "Voicemeeter login result after running: " +
                                   std::to_string(loginResult));
        ...
        if (loggedIn) {
            ...
            Logger::Instance().Log(
                LogLevel::DEBUG,
                "GetVoicemeeterType result after running: " + std::to_string(hr) +
                    ", Type: " + std::to_string(vmType));
            if (hr != 0) {
...
void VoicemeeterManager::ShutdownCommand() {
    Logger::Instance().Log(LogLevel::DEBUG,
                           "Sending shutdown command to Voicemeeter.");
...
void VoicemeeterManager::RestartAudioEngine(int beforeRestartDelay,
    ...
    Logger::Instance().Log(LogLevel::DEBUG,
                           "Voicemeeter audio engine restarted.");
}
 ...
 */
bool VoicemeeterManager::GetDebugMode() { return debugMode; }
 ...
 */
VoicemeeterAPI &VoicemeeterManager::GetAPI() { return vmrAPI; }
...
void VoicemeeterManager::ListAllChannels() {
    ...
    auto PrintParameter = [&](const std::string &param, const std::string &type,
        ...
        if (result == 0 && strlen(label) > 0 && strlen(label) < sizeof(label)) {
            Logger::Instance().Log(LogLevel::INFO, "| " + std::to_string(index) +
                                                       " | " + label + " | " + type +
                                                       " |");
        } else {
            Logger::Instance().Log(LogLevel::INFO, "| " + std::to_string(index) +
...
void VoicemeeterManager::ListInputs() {
    try {
        ...
        Logger::Instance().Log(LogLevel::INFO,
                               "Available Voicemeeter Virtual Inputs:");
        ...
        Logger::Instance().Log(LogLevel::ERR, "Error listing Voicemeeter inputs: " +
                                                  std::string(ex.what()));
    }
}
...
void VoicemeeterManager::ListOutputs() {
    try {
        ...
        Logger::Instance().Log(LogLevel::INFO,
                               "Available Voicemeeter Virtual Outputs:");
        ...
        Logger::Instance().Log(LogLevel::ERR,
                               "Error listing Voicemeeter outputs: " +
                                   std::string(ex.what()));
    }
}
