 ...
 */
Logger::Logger() : logLevel(LogLevel::INFO), fileLoggingEnabled(false) {}
...
void Logger::Log(LogLevel level, std::string_view message) {
    ...
    std::ostream &output =
        (fileLoggingEnabled && logFile.is_open()) ? logFile : std::cout;
    ...
    output << "[" << time_buffer << "." << std::setfill('0') << std::setw(3)
           << milliseconds.count() << "] " << levelStr << message << std::endl;
}